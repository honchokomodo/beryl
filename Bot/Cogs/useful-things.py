...
class usefulThings(commands.Cog):
    ...
    @slash_command(
        name="write",
        description="(): save userdata apparently",
        guild_ids=[978909341665079366],
    )
    async def writeUserData(self, ctx):
        # if ctx.author.bot is not True:  # THIS IS HORRIBLE CHANGE THIS ASAP
        #     await ctx.respond("you are not the bot host")
        #     return
